...
class OrderManagement(QWidget):
    ...
    def init_ui(self):
        ...
        # 搜索板件编号（靠近刷新按钮）
        self.component_search_edit = QLineEdit()
        self.component_search_edit.setPlaceholderText("搜索板件编号...")
        self.component_search_edit.returnPressed.connect(self.search_component)
        toolbar_layout.addWidget(self.component_search_edit)
        
        self.component_search_btn = QPushButton("搜索板件")
        self.component_search_btn.clicked.connect(self.search_component)
        toolbar_layout.addWidget(self.component_search_btn)
        ...
        info_stats_layout.addWidget(order_info_group)
        
        # 订单统计显示框（板件数量、房间数）
        order_stats_group = QGroupBox("订单统计")
        stats_layout = QGridLayout(order_stats_group)
        stats_layout.addWidget(QLabel("板件数量:"), 0, 0)
        self.component_count_label = QLabel("0")
        stats_layout.addWidget(self.component_count_label, 0, 1)
        stats_layout.addWidget(QLabel("房间数:"), 1, 0)
        self.room_count_label = QLabel("0")
        stats_layout.addWidget(self.room_count_label, 1, 1)
        info_stats_layout.addWidget(order_stats_group)
        # 设置并排区域等宽显示
        try:
            info_stats_layout.setStretch(0, 1)
            info_stats_layout.setStretch(1, 1)
        except Exception:
            pass
        right_layout.addLayout(info_stats_layout)
    ...
    def load_order_details(self, order_id):
        ...
        for comp in components:
            ...
            if room:
                room_set.add(room)
        self.component_count_label.setText(str(comp_count))
        self.room_count_label.setText(str(len(room_set)))
...
失败: {result['error_count']}"""
    ...
    def delete_order(self):
        ...
        try:
            ...
            if hasattr(self, 'component_count_label'):
                self.component_count_label.setText("0")
            if hasattr(self, 'room_count_label'):
                self.room_count_label.setText("0")
    ...
    def search_component(self):
        """搜索板件编号，显示所属订单号，并选中订单"""
        text = self.component_search_edit.text().strip()
        if not text:
            QMessageBox.warning(self, "提示", "请输入板件编号")
            return
        try:
            conn = db.get_connection()
            cursor = conn.cursor()
            # 精确搜索
            cursor.execute('''
                SELECT c.component_code, o.order_number, o.id
                FROM components c JOIN orders o ON c.order_id = o.id
            ...
            if rows:
                # 汇总订单号
                order_numbers = sorted({r[1] for r in rows})
                msg = "找到板件编号，所属订单号：\n" + "\n".join(order_numbers[:10])
                if len(order_numbers) > 10:
                    msg += f"\n... 还有 {len(order_numbers) - 10} 个"
                QMessageBox.information(self, "搜索结果", msg)
                # 自动选中第一个匹配订单并滚动显示
                target_order_id = rows[0][2]
                for i in range(self.orders_table.rowCount()):
                    item = self.orders_table.item(i, 0)
                    if item and item.data(Qt.UserRole) == target_order_id:
                        self.orders_table.selectRow(i)
                        self.orders_table.scrollToItem(item)
                        break
            else:
                QMessageBox.information(self, "未找到", f"未找到包含板件编号 {text} 的订单")
        except Exception as e:
