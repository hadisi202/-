...
class PalletManagement(QWidget):
    ...
    def init_ui(self):
        ...
        self.pallets_table.setHorizontalHeaderLabels(['托盘编号', '序号', '类型', '包裹数', '容量', '状态'])
    ...
    def load_pallets(self):
        ...
        try:
            ...
            for row, pallet in enumerate(pallets):
                # 列对应：0托盘编号 | 1序号 | 2类型 | 3包裹数 | 4容量 | 5状态
                ...
                # 容量（来自设置 pallet_capacity，若无则留空）
                capacity_text = ''
                try:
                    from database import db as _db_inst
                    cap_setting = _db_inst.get_setting('pallet_capacity', '')
                    capacity_text = str(int(cap_setting)) if str(cap_setting).strip().isdigit() else ''
                except Exception:
                    capacity_text = ''
                self.pallets_table.setItem(row, 4, QTableWidgetItem(capacity_text))  # 容量

                # 状态中文
                status_text = ""
                status_text = pallet_status_cn(pallet[3])
                self.pallets_table.setItem(row, 5, QTableWidgetItem(status_text))  # 状态
    ...
    def on_pallet_selected(self):
        ...
        if current_row >= 0:
            ...
            # 根据托盘状态控制包裹操作
            status_item = self.pallets_table.item(current_row, 5)
            ...
            if hasattr(self, 'delete_package_btn'):
                self.delete_package_btn.setEnabled(not is_sealed)
        else:
            self.current_pallet_id = None
    ...
    def delete_packages(self):
        ...
        try:
            ...
            cur.execute('SELECT status FROM pallets WHERE id = ?', (self.current_pallet_id,))
            row = cur.fetchone()
            if row and row[0] in ('sealed', 'closed'):
                conn.close()
                QMessageBox.information(self, "提示", "封托或关闭后不可操作包裹")
                return
            ...
            if not numbers:
                conn.close()
                QMessageBox.information(self, "提示", "请在托盘内选择要删除的包裹")
                return

            # 业务限制：托盘中的包裹不可被删除
            QMessageBox.information(self, "提示", "托盘中的包裹不可删除。如需调整，请移动到其他托盘。")
            conn.close()
        except Exception as e:
            QMessageBox.critical(self, "错误", f"删除包裹失败：{str(e)}")
        ...
        try:
            ...
            if search_text:
                count_sql = '''
                    ...
                    LEFT JOIN orders o ON p.order_id = o.id
                    WHERE p.order_id = ? AND (p.package_number LIKE ? OR o.order_number LIKE ?)
                '''
                cursor.execute(count_sql, (self.current_order_id, f'%{search_text}%', f'%{search_text}%'))
            else:
                cursor.execute('SELECT COUNT(*) FROM packages WHERE order_id = ?', (self.current_order_id,))
            total = cursor.fetchone()[0] or 0
            self.packages_total_pages = max(1, (total + page_size - 1) // page_size)
    ...
    def update_pallet_info_display(self):
        ...
        try:
            ...
            if pallet_info:
                ...
                # 更新托盘基本信息（紧凑条）
                self.info_number_label.setText(str(pallet_number))
                self.info_type_label.setText(pallet_type_text)
                self.info_status_label.setText(status_text)
                self.info_count_label.setText(str(package_count))
    ...
    def delete_pallet(self):
        ...
        try:
            ...
            if package_count > 0:
                ...
                # 将包裹从托盘中移出
                cursor.execute('UPDATE packages SET pallet_id = NULL WHERE pallet_id = ?', (self.current_pallet_id,))
            else:
                reply = QMessageBox.question(self, "确认删除", 
    ...
    def remove_package_from_pallet(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                cursor.execute('UPDATE packages SET pallet_id = NULL WHERE id = ?', (package_id,))
                conn.commit()
                conn.close()
    ...
    def update_selected_pallet_info_panel(self):
        ...
        try:
            ...
            if row is None or row < 0:
                ...
                self.pkg_info_capacity.setText("")
                self.pkg_info_status.setText("")
                self.pkg_info_order.setText("")
                self.pkg_info_created.setText("")
                return
            ...
            # 容量
            self.pkg_info_capacity.setText(t(4))
            self.pkg_info_status.setText(t(5))  # 状态
